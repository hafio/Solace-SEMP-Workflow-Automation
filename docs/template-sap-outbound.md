...
**File:** [examples/templates/sap-outbound.yaml](../examples/templates/sap-outbound.yaml)

**Message direction:** Solace -> SAP (broker receives messages and delivers to a downstream SAP system).

**Use case:** Provision the broker resources needed for a specific SAP outbound topic — shared client profile, per-user ACL profile, client username, and the queue set (service + mirror + DMQ) with topic subscriptions.
...
|---|---|---|
| `sap-outbound.new-seq` | Create the full outbound stack with **sequential** delivery | `service_queue_max_redelivery: 0` (retry forever, never give up) |
| `sap-outbound.new-non-seq` | Create the full outbound stack with **concurrent/retry** delivery | `service_queue_max_redelivery: 5` (route to DMQ after 5 retries) |
| `sap-outbound.delete` | Tear down all resources for a given outbound topic | Removes queues, topic exception, username, and per-user ACL profile |
...
| `system` | Source/downstream system name | `NATS` |
| `system_topic` | Business topic identifier | `GCM.FIANA.LOT-TURNKEY-COST` |
| `service_queue_owner` | Client username that owns the service queue | `MI-NATS-CLIENT` |
| `non_service_queue_owner` | Client username that owns the mirror queue and DMQ | `ADMIN-USER` |
| `aem_client_username` | Client username to create for this topic | `MI-NATS-CLIENT` |
...
workflows:
  - template: "sap-outbound.new-seq"
    inputs:
      ...
      service_queue_owner: "MI-NATS-CLIENT"
      non_service_queue_owner: "ADMIN-USER"
      aem_client_username: "MI-NATS-CLIENT"
```
...
| ACL profile | `acl-it-user-MI-NATS-CLIENT` |
| Client username | `MI-NATS-CLIENT` |
| Publish topic exception | `FCM/SAP/AIF/GCM.FIANA.LOT-TURNKEY-COST` |
| Service queue | `TO-HQ-NATS-GCM.FIANA.LOT-TURNKEY-COST` |
| Mirror queue | `MIRROR/TO-HQ-NATS-GCM.FIANA.LOT-TURNKEY-COST` |
| DMQ | `DMQ/TO-HQ-NATS-GCM.FIANA.LOT-TURNKEY-COST` |
