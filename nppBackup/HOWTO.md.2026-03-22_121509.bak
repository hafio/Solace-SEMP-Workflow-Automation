...
**Use standalone executable (.zip)**

If you received a `semp-workflow.zip` bundle, it is fully self-contained — all Python dependencies (`click`, `jinja2`, `requests`, `colorama`, etc.) are bundled inside. Only a Python 3.10+ interpreter is required on the target machine; no `pip install` needed.

```bash
python semp-workflow.zip --help
```

On first use, export the bundled templates to a local directory for customisation:
```bash
...
└── workflow-templates/       # Template directory
    ├── sap-inbound.yaml      # SAP inbound workflow templates
    └── sap-outbound.yaml     # SAP outbound workflow templates
```

- **`config.yaml`**: defines the SEMP connection, shared `global_vars`, and the list of workflows to execute
- **`workflow-templates/`**: each YAML file holds one or more named templates, each defining input variables and an action sequence
...
| `--templates-dir` | `-t` | Override the workflow templates directory |
| `--dry-run` / `--check` | | Show what would be done without making changes |
| `--fail-fast` | `-f` | Stop execution on the first failure |
...
Check config and template syntax without executing anything:
...
# Export module docs to a Markdown file
semp-workflow list-modules --output all-modules.md
```

### 7.4 Export Bundled Templates

(Only available when running from a `.zip` bundle)
...
|---|---|---|
| `domain` | Business domain | `HQ` |
| `system` | System name | `NATS` |
| `system_topic` | Topic identifier | `GCM.FIANA.LOT-COST` |
...
> All optional inputs have defaults. Override them in the `config.yaml` workflow entry. See the `optional:` block in each template file for the full list.
...
For full parameter details, run `semp-workflow list-modules` or see `all-modules.md`.
...
### Scenario 2: Batch-create multiple workflows

Add multiple entries to the `workflows` list for batch execution:
...
### Scenario 3: Centralise backend connection settings in global_vars
...
> **Note**: The inbound delete template deletes the RDP first, then the queues — the correct order to avoid orphaned resources.
...
- If using a `.zip` bundle without an external template directory, run `python semp-workflow.zip init` to export bundled templates
