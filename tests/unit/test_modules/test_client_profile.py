...
from semp_workflow.modules.client_profile import ClientProfileAdd, ClientProfileDelete, ClientProfileUpdate
...
class TestClientProfileUpdate:
    def test_ok_when_exists(self, mock_client, update_module):
        ...
        result = update_module.execute(
            mock_client,
            {"clientProfileName": "CP1", "allowGuaranteedMsgSendEnabled": "true"},
        )
        assert result.status == ResultStatus.OK
    ...
    def test_failed_when_not_exists(self, mock_client, update_module):
        mock_client.exists.return_value = (False, None)
        result = update_module.execute(
            mock_client, {"clientProfileName": "CP1", "maxSubscriptionCount": "100"}
        )
        assert result.status == ResultStatus.FAILED

    def test_skipped_when_no_fields(self, mock_client, update_module):
        mock_client.exists.return_value = (True, {})
        result = update_module.execute(mock_client, {"clientProfileName": "CP1"})
        assert result.status == ResultStatus.SKIPPED
    ...
    def test_profile_name_not_in_payload(self, mock_client, update_module):
        mock_client.exists.return_value = (True, {})
        ...
        assert "clientProfileName" not in payload

    def test_bool_fields_coerced(self, mock_client, update_module):
        mock_client.exists.return_value = (True, {})
        update_module.execute(
            mock_client,
            {
        ...
        payload = mock_client.update.call_args[0][1]
        assert payload["allowGuaranteedMsgSendEnabled"] is True
        assert payload["allowBridgeConnectionsEnabled"] is False

    def test_int_fields_coerced(self, mock_client, update_module):
        mock_client.exists.return_value = (True, {})
        update_module.execute(
            mock_client,
            {"clientProfileName": "CP1", "maxSubscriptionCount": "500"},
        )
        payload = mock_client.update.call_args[0][1]
        assert payload["maxSubscriptionCount"] == 500

    def test_failed_on_empty_name(self, mock_client, update_module):
        result = update_module.execute(mock_client, {"clientProfileName": ""})
        assert result.status == ResultStatus.FAILED

    def test_failed_on_name_too_long(self, mock_client, update_module):
        result = update_module.execute(mock_client, {"clientProfileName": "x" * 33})
        assert result.status == ResultStatus.FAILED
